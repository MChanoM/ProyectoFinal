  ...
  const cambiarEstado = async () => {
    try {
        ...
        if (consulta.status === 200) {
          Swal.fire("Estado modificado!", "El estado se modificó correctamente!","success");
    ...
    <tr>
      ...
      <td>
        {props.item.role.map((item) => {
          return <span key={item._id} className="badge badge-danger mx-1">{item.name}</span>;
        })}
      </td>
      <td>{props.item.userActive ? "Activo" : "Inactivo"}</td>
      <td>{props.item.sessionState ? "Logueado" : "No Logueado"}</td>
      <td>
        <Link to={`/usuario/editar/${props.item._id}`} className="mr-2 text-dark btnAcciones">
